...
const GridSingle = ({ product,handleAddedCart }) => {
  ...
  return (
    <div className="border rounded-xl shadow-xl p-6 space-y-4 font-display hover:scale-102 transition-all duration-200">
      ...
      <button className={myBtnStyle + "w-full rounded-4xl p-3 text-white font-bold hover:scale-105 transition-all duration-200"} onClick={()=> handleAddedCart(product, price)}>Buy Now</button>
    </div>
  );
};
